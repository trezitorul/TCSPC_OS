...
if __name__ == "__main__":
    ...
    cam.connect()
    # img = cam.acquire(10)
    # image_array = ((img - np.min(img)) / np.ptp(img) * 255).astype(np.uint8)
    ...
    while cam._is_open:
        ...
        # rgb = cv2.cvtColor(image_array, cv2.COLOR_GRAY2BGR)
...
#     def open(self, bit_depth=8, roi_shape=(1024, 1024), roi_pos=(0, 0), camera="ThorCamel FS", exposure=0.01,
#              frametime=10.0):
...
#         '''
#         # mode = self.uc480.is_SetExternalTrigger(self.handle, )

#         # print("test")
#         checker = self.uc480.is_CaptureVideo(self.handle, 0x0000)
#         time.sleep(10)
#         print("captured: " + str(checker))
#         # print("mode: " + str(mode))
#         # print(self.handle)
